...
public abstract class PageSelector<T> : MonoBehaviour
{
    ...
    public void Open()
    {
        InputSwitcher.instance.input.actions.FindAction("ChangePageLeft").started += SetToPrevPage;
        InputSwitcher.instance.input.actions.FindAction("ChangePageRight").started += SetToNextPage;
        panel.Open();
        SetCurrentPanel(0);

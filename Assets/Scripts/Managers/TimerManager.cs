...
public class TimerManager : MonoSingleton<TimerManager>, IPause
{
    ...
    public string AddTimer(float timer, Action OnEndTimer)
    {
        string timerKey = currentTimer.ToString();
        cdModule.AddCD(timerKey, OnEndTimer, timer);
        currentTimer += 0.01f;
    ...
    public string AddTimer(float timer, Action OnEndTimer, string key)
    {
        cdModule.AddCD(key, OnEndTimer, timer);
